...
class Punk(object):
    ...
    def decode(self, input_file, output_file):
        ...
        self._header = open(output_file+'_base',  'ab+')
    ...
    def encode(self, input_file, payload_bytes):

        self._mode = 'encode'
        self._file = open(input_file, 'rb+')
        self._bytes_to_hide = payload_bytes
    ...
    def _read_bytes_as_hex(self, position):
        ...
        return self._read_bytes(position).hex()

    def _read_bytes_as_ascii(self, position):

        return self._read_bytes(position).decode('ascii')
    ...
    def _inject_punk_chunk(self):
        ...
        tmp_bytes.extend(bytearray(self._PUNK_CHUNK_TYPE,'utf8'))
        ...
        # And the type
        self._file.write(bytearray(self._PUNK_CHUNK_TYPE,'utf8'))
        ...
        # # Then the chunk type.
        # self._file.write(bytearray(self._END_CHUNK_TYPE,'utf8'))
    ...
    def _read_next_chunk(self):
        ...
        if self._mode == 'decode' and chunk_type == self._PUNK_CHUNK_TYPE:
            ...
            print("Found a bLOb chunk", len(content), "bytes. Writing to file")
